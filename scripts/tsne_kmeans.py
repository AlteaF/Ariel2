import numpy as np
import matplotlib.pyplot as plt
import argparse
from sklearn.manifold import TSNE
from sklearn.cluster import KMeans
from collections import defaultdict, Counter
from PIL import Image
from matplotlib.offsetbox import OffsetImage, AnnotationBbox
from matplotlib.patches import Patch
import os
import re
import numpy.random as npr

# ---- Edit this mapping in-code ----
LABEL_MAP = {
        1: "Starfish",
        2: "Crab",
        3: "Black goby",
        4: "Wrasse",
        5: "Two-spotted goby",
        6: "Cod",
        7: "Painted goby",
        8: "Sand eel",
        9: "Whiting"
    }
# -----------------------------------

IMAGE_BASE_DIR = "/Users/alteafogh/Documents/ITU/Research_project/Finding_A_Nemo/dataset/cropped/cropped_train"

def load_npz_data(npz_path):
    data = np.load(npz_path, allow_pickle=True)
    embeddings = data["embeddings"]
    image_paths = data["image_paths"]
    return embeddings, image_paths

def extract_numeric_label_from_path(path):
    """
    Extract the *last single digit* right before the extension.
    Example: 'dir/foo_123_7.jpg' -> 7
    Raises ValueError if nothing found.
    """
    filename = os.path.basename(path)
    # Search for a single digit (\d) that is immediately followed by
    # a dot and one of the extensions (non-capturing group (?:...)).
    match = re.search(r"(\d)(?=\.(?:jpe?g|png)$)", filename, flags=re.IGNORECASE)
    if not match:
        raise ValueError(f"Could not extract single numeric label from filename: {filename}")
    return int(match.group(1))

def resolve_image_path(img_path, npz_base_dir):
    """
    Resolve image paths using the global IMAGE_BASE_DIR first.
    """
    # 1) Try inside IMAGE_BASE_DIR
    candidate = os.path.join(IMAGE_BASE_DIR, os.path.basename(img_path))
    if os.path.exists(candidate):
        return candidate

    # 2) Try as provided
    if os.path.exists(img_path):
        return img_path

    # 3) Try relative to the .npz directory
    candidate2 = os.path.join(npz_base_dir, img_path)
    if os.path.exists(candidate2):
        return candidate2

    # 4) Try basename inside the .npz directory
    candidate3 = os.path.join(npz_base_dir, os.path.basename(img_path))
    if os.path.exists(candidate3):
        return candidate3

    # Not found
    return None

def run_tsne_kmeans(
    embeddings, image_paths, n_clusters,
    output_path=None, samples_per_label=1,
    perplexity=30, random_state=42, show_images=True, npz_base_dir="."
):
    """
    1. Runs t-SNE on embeddings to reduce to 2 dimensions.
    2. Runs K-Means clustering on the 2D results.
    3. Plots the 2D points colored by K-Means cluster assignment.
    4. Returns the K-Means cluster labels.
    """
    print(f"Running t-SNE on {len(embeddings)} vectors (perplexity={perplexity})...")
    tsne = TSNE(n_components=2, perplexity=perplexity, random_state=random_state)
    tsne_results = tsne.fit_transform(embeddings)

    print(f"Running K-Means on t-SNE results (n_clusters={n_clusters}, random_state={random_state})...")
    kmeans = KMeans(n_clusters=n_clusters, random_state=random_state, n_init=10)
    # The cluster assignments (0, 1, ..., n_clusters-1) are our new "labels"
    cluster_labels = kmeans.fit_predict(tsne_results)
    
    # Map cluster index (int) to a string for plotting/counting
    mapped_labels = [f"Cluster {c}" for c in cluster_labels]
    
    counts = Counter(mapped_labels)
    print("Counts per cluster:")
    for lab, c in counts.items():
        print(f"  {lab}: {c}")
    
    # --- Plotting ---
    unique_labels = sorted(counts.keys(), key=lambda x: int(x.split()[-1]))
    cmap = plt.get_cmap('tab20')
    label_to_color = {lab: cmap(i % cmap.N) for i, lab in enumerate(unique_labels)}

    plt.figure(figsize=(12, 8))

    for lab in unique_labels:
        indices = [i for i, l in enumerate(mapped_labels) if l == lab]
        coords = tsne_results[indices]
        plt.scatter(coords[:, 0], coords[:, 1],
                    s=20, alpha=0.7, label=f"{lab} ({len(indices)})",
                    color=label_to_color[lab])

    legend_handles = [Patch(color=label_to_color[lab], label=f"{lab} ({counts[lab]})") for lab in unique_labels]
    plt.legend(handles=legend_handles, title=f"K-Means Clusters (k={n_clusters})", bbox_to_anchor=(1.05, 1), loc='upper left')

    # Add thumbnails (unchanged logic)
    if show_images:
        print("Attempting to add thumbnails...")
        grouped = defaultdict(list)
        for i, lab in enumerate(mapped_labels):
            grouped[lab].append(i)

        rng = npr.default_rng(random_state)
        missing_images = 0
        added_images = 0

        for lab, indices in grouped.items():
            sample_count = min(samples_per_label, len(indices))
            chosen = rng.choice(indices, size=sample_count, replace=False)
            for idx in chosen:
                img_path_candidate = resolve_image_path(image_paths[idx], npz_base_dir)
                if img_path_candidate is None:
                    missing_images += 1
                    continue
                try:
                    img = Image.open(img_path_candidate).convert('RGB')
                    img.thumbnail((64, 64), Image.Resampling.LANCZOS)
                    imagebox = OffsetImage(np.asarray(img), zoom=1)
                    ab = AnnotationBbox(imagebox,
                                        (tsne_results[idx, 0], tsne_results[idx, 1]),
                                        frameon=False, pad=0.3)
                    plt.gca().add_artist(ab)
                    added_images += 1
                except Exception as e:
                    missing_images += 1
                    pass

        print(f"Thumbnails: added {added_images}, missing {missing_images}")

    plt.title(f"t-SNE Reduction with K-Means Clustering (k={n_clusters})")
    plt.xlabel("t-SNE Dimension 1")
    plt.ylabel("t-SNE Dimension 2")
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.tight_layout()

    if output_path:
        # Create output directory if it doesn't exist
        os.makedirs(os.path.dirname(output_path) or '.', exist_ok=True) 
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"Saved figure to {output_path}")
    else:
        plt.show()
        
    # --- Return the cluster labels for saving ---
    return cluster_labels


def main():
    parser = argparse.ArgumentParser(description="t-SNE on embeddings followed by K-Means clustering, with result saving.")
    parser.add_argument("--embedding_file", type=str, required=True, help="Path to .npz file with 'embeddings' and 'image_paths'")
    parser.add_argument("--n_clusters", type=int, default=9, help="Number of clusters for K-Means.")
    parser.add_argument("--plot_output_file", type=str, default='tsne_kmeans_plot.png', help="Output path for the scatter plot image.") # Renamed for clarity
    parser.add_argument("--data_output_file", type=str, default='clustered_tsne_embeddings.npz', help='Output .npz file to store original data and cluster labels.') # <-- Added argument
    parser.add_argument("--samples_per_label", type=int, default=3)
    parser.add_argument("--perplexity", type=float, default=30.0)
    parser.add_argument("--show_images", action="store_true")
    parser.add_argument("--random_state", type=int, default=42)

    args = parser.parse_args()

    embeddings, image_paths = load_npz_data(args.embedding_file)
    npz_base_dir = os.path.dirname(os.path.abspath(args.embedding_file)) or "."
    
    # 1. Run t-SNE and K-Means and get the cluster labels
    cluster_labels = run_tsne_kmeans(
        embeddings=embeddings,
        image_paths=image_paths,
        n_clusters=args.n_clusters,
        output_path=args.plot_output_file,
        samples_per_label=args.samples_per_label,
        perplexity=args.perplexity,
        show_images=args.show_images,
        npz_base_dir=npz_base_dir,
        random_state=args.random_state
    )

    # 2. Save the results in the requested format
    print("\n💾 Saving clustered data...")
    # The saved data matches the structure you provided: embeddings, image_paths, cluster_labels
    np.savez(
        args.data_output_file,
        embeddings=embeddings,
        image_paths=image_paths,
        cluster_labels=cluster_labels
    )
    print(f"✅ Saved original embeddings, image paths, and cluster labels to **{args.data_output_file}**")


if __name__ == "__main__":
    main()